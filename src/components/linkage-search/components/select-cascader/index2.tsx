...
const SelectCascader: React.FC<SelectCascaderProps> = (props: SelectCascaderProps) => {
    ...
    const onClicknewArr = (item,levelIndex) => {
        if(levelIndex==0) onClickCategory(item);
        if(levelIndex==1) onClickDiscipline(item);
        if(levelIndex==2) onClickSpeciality(item);
     //   if (level == levelIndex+1) props.onChange && props.onChange(majorResultJson);
    };
            ...
            <AtTabs current={current} tabList={tabList} onClick={handleClickAtTabs}>
                {
                    ...
                    localValue.map(((item: any, index: number) => {
                        ...
                        let newArr = [category,discipline,speciality]
                        return (
                            <AtTabsPane current={current} index={index}>
                                <View className="specialty-box">
                                    {
                                        Array.from(Array(level).keys()).map((levelItem,levelIndex)=>(

                                        <View className="study-type-box">
                                            {
                                                newArr[levelIndex].map((newArrItem: any) => (
                                                    <View
                                                        className={`study-type-list ${newArrItem.checked ? 'actived' : ''}`}
                                                        onClick={() => onClicknewArr(newArrItem,levelIndex)}>
                                                        <Text>{newArrItem.objectName}&emsp;</Text>
